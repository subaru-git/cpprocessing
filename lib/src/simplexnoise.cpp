...
namespace {
constexpr int32_t fastfloor(float f) {
...
constexpr uint8_t perm[256] = {
    ...
    180};
constexpr uint8_t hash(int32_t i) { return perm[static_cast<uint8_t>(i)]; }
constexpr float grad(int32_t hash, float x) {
  const int32_t h = hash & 0x0F;
  float grad = 1.0f + (h & 7);
  if ((h & 8) != 0) grad = -grad;
  return (grad * x);
}
constexpr float grad(int32_t hash, float x, float y) {
